...
# 2.1. identical function : 두 개의 객체가 "정확히" 같은지 비교 -> 별로 쓸모 없음
# 2.2. all.equal function : 미미한 오차는 무시.
...
test(a = 10, b = 20) # a,b 둘다 선언했으므로, print 미출력
...
# 첫번째 인자에 반복 횟수를 넣고, 두번째 인자에 반복문의 내용을 넣어주면, return vector 를 생성해주는
...
matrix1 <- aperm(array(c(1:24), dim = c(4, 3, 2)), perm = c(2, 1, 3)) # perm : 차원, 열, 행의 순서 
